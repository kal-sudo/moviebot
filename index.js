...
bot.start((ctx) =>
  ctx.reply(
    "Hey There!This is your personal movie hunter!Tell me what you're looking for today.You may use genre,release date or any combination of these to receive the appropriate recommendations"
  )
);
  ...
  try {
    ...
    if (recs.length > 0) {
      ...
      for (let i = 0; i < Math.min(recs.length, 3); i++) {
        ...
        try {
          const truview_reply = await truview(recs[i].title, apiOptions);
          ctx.replyWithPhoto(
            { url: `https://image.tmdb.org/t/p/w500/${recs[i].poster_path}` },
            {
              caption:
    ...
    if (error.code === "ECONNRESET") {
      console.error(
        "Caught ECONNRESET error: The connection was reset by the peer."
      );
